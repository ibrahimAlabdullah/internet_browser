...
public class QueryParser {
    ...
    private void byId() {
        ...
        HttpConnection.notEmpty(id);
        evals.add(new Evaluator.Id(id));
    }

    private void byClass() {
        String className = tq.consumeCssIdentifier();
        HttpConnection.notEmpty(className);
    ...
    private void byTag() {
        ...
        HttpConnection.notEmpty(tagName);

        // namespaces: wildcard match equals(tagName) or ending in ":"+tagName
        if (tagName.startsWith("*|")) {
            evals.add(new CombiningEvaluator.Or(new Evaluator.Tag(Evaluator.lowerCase(tagName).trim()), new Evaluator.TagEndsWith(Evaluator.lowerCase(tagName.replace("*|", ":")).trim())));
        } else {
            // namespaces: if tags name is "abc:def", selector must be "abc|def", so flip:
            if (tagName.contains("|"))
	...
	private void cssNthChild(boolean backwards, boolean ofType) {
		String argS = Evaluator.lowerCase(tq.chompTo(")")).trim();
    ...
    private int consumeIndex() {
        ...
        HttpConnection.isTrue(StringUtil.isNumeric(indexS), "Index must be numeric");
    ...
    private void has() {
        ...
        HttpConnection.notEmpty(subQuery, ":has(el) subselect must not be empty");
    ...
    private void contains(boolean own) {
        ...
        HttpConnection.notEmpty(searchText, ":contains(text) query must not be empty");
        if (own)
    ...
    private void containsData() {
        ...
        String searchText = TokenQueue.unescape(tq.chompBalanced('(', ')'));
        HttpConnection.notEmpty(searchText, ":containsData(text) query must not be empty");
    ...
    private void matches(boolean own) {
        ...
        HttpConnection.notEmpty(regex, ":matches(regex) query must not be empty");
    ...
    private void not() {
        tq.consume(":not");
        String subQuery = tq.chompBalanced('(', ')');
        HttpConnection.notEmpty(subQuery, ":not(selector) subselect must not be empty");
