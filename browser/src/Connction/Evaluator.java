...
/**
 * Evaluates that an tags matches the selector.
 */
public abstract class Evaluator {
    ...
    /**
     * Test if the tags meets the evaluator's requirements.
     *
     * @param root    Root of the matching subtree
     * @param element tested tags
    ...
    public static final class AttributeStarting extends Evaluator {
        ...
        public AttributeStarting(String keyPrefix) {
            ...
            this.keyPrefix =keyPrefix.toLowerCase(Locale.ENGLISH);// lowerCase(keyPrefix);
        }

        @Override
        public boolean matches(Element root, Element element) {
            List<nodes.Attribute> values = element.attributes().asList();
            for (nodes.Attribute attribute : values) {
    ...
    public static final class AttributeWithValueMatching extends Evaluator {
        ...
        public AttributeWithValueMatching(String key, Pattern pattern) {
           // this.key = Normalizer.normalize(key);
            this.key=key.toLowerCase(Locale.ENGLISH).trim();
    ...
    public abstract static class AttributeKeyPair extends Evaluator {
        ...
        public AttributeKeyPair(String key, String value) {
            if (key == null || key.length() == 0||value== null || value.length() == 0)
                throw new IllegalArgumentException("String must not be empty");
            this.key = lowerCase(key).trim();
            if (value.startsWith("\"") && value.endsWith("\"")
                    || value.startsWith("'") && value.endsWith("'")) {
                value = value.substring(1, value.length()-1);
            }
            this.value = lowerCase(value).trim();
        }
    }

    /**
     * Evaluator for any / all tags matching
     */
    ...
    public static final class IsEmpty extends Evaluator {
		...
		public boolean matches(Element root, Element element) {
            ...
            for (Node n : family) {
                if (!(n instanceof Comment || n instanceof DocumentType)) return false;
            }
        	return true;
		}
    	@Override
        ...
        public boolean matches(Element root, Element element) {
            ...
            for (TextNode textNode : textNodes) {
                Element pel = new Element(
                    parserr.Tag.valueOf(element.tagName()), element.baseUri(), element.attributes());
